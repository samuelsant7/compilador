...
O analisador léxico, implementado com `ply.lex`, é responsável por ler o código-fonte e convertê-lo em uma sequência de *tokens*.
...
### 2.3. Análise Semântica (`src/semantic_analyzer.py`)
...
O compilador foi testado com um conjunto de casos válidos e inválidos, executados através do *script* `src/main.py`.

### 3.1. Teste Válido (Exemplo de Saída)

O código de teste a seguir foi compilado com sucesso:
...
### 3.2. Teste Inválido (Exemplo de Erro Semântico)
